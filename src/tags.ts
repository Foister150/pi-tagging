 ...
 * /tag <t…>  — quick-add one or more path-style tags (e.g. /tag project/frontend).
 * /tag pick  — choose any session, then edit its tags.
 *
 * /tag-view  — browse all sessions as a tag tree. Top level shows tags like
 *              "project", "extension-development", plus an "etc" bucket for
...
interface TreeNode {
  name: string;
  fullPath: string; // tag path to this node ("project/frontend")
  ...
  while (true) {
    ...
    if (choice === ADD) {
      const entered = await ctx.ui.input(
        "Add tag(s) — space-separated, path-style e.g. project/frontend:",
      );
